...
const files = fs.readdirSync(dir).filter(f => f.startsWith('intonationMimic_') && f.endsWith('.json'));

console.log(`Found ${files.length} Intonation Mimic curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'intonationMimic') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'intonationMimic', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "word", "textToSpeak", "options", "correctAnswerIndex", "intonationMap", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Intonation Mimic curriculum is 100% PERFECT!`);
  process.exit(0);
}
