...
const files = fs.readdirSync(dir).filter(f => f.startsWith('minimalPairs_') && f.endsWith('.json'));

console.log(`Found ${files.length} Minimal Pairs curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'minimalPairs') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'minimalPairs', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "word1", "word2", "ipa1", "ipa2", "textToSpeak", "correctAnswer", "options", "correctAnswerIndex", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Minimal Pairs curriculum is 100% PERFECT!`);
  process.exit(0);
}
