...
const files = fs.readdirSync(dir).filter(f => f.startsWith('pronounResolution_') && f.endsWith('.json'));

console.log(`Found ${files.length} Pronoun Resolution curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'pronounResolution') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'pronounResolution', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "question", "targetWord", "sentence", "options", "correctAnswerIndex", "correctAnswer", "hint", "explanation"];
    ...
    if (q.question) {
      ...
      if (allSentences.has(normalized)) {
        duplicates.push(`Duplicate sentence: "${q.question}" in ${file} (Quest ID: ${q.id})`);
      }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Pronoun Resolution curriculum is 100% PERFECT!`);
  process.exit(0);
}
