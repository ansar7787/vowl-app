...
const files = fs.readdirSync(dir).filter(f => f.startsWith('relativeClauses_') && f.endsWith('.json'));

console.log(`Found ${files.length} Relative Clauses curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'relativeClauses') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'relativeClauses', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Relative Clauses curriculum is 100% PERFECT!`);
  process.exit(0);
}
