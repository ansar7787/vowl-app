...
const files = fs.readdirSync(dir).filter(f => f.startsWith('summarizeStoryWriting_') && f.endsWith('.json'));

console.log(`Found ${files.length} Summarize Story curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'summarizeStoryWriting') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'summarizeStoryWriting', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "story", "options", "correctOrder", "hint", "explanation"];
    ...
    // Verify correctOrder is an array of size 3 and contains valid indices
    if (q.correctOrder) {
      if (!Array.isArray(q.correctOrder)) {
        missingFields.push(`correctOrder is not an array in quest ${q.id} in ${file}`);
      } else if (q.correctOrder.length !== 3) {
        missingFields.push(`correctOrder must have exactly 3 index elements in quest ${q.id} in ${file}`);
      } else {
    ...
    // Check story uniqueness
    if (q.story) {
      const normalized = q.story.trim().toLowerCase();
      if (allStories.has(normalized)) {
        duplicates.push(`Duplicate story: "${q.story.substring(0, 40)}..." in ${file} (Quest ID: ${q.id})`);
      }
      allStories.add(normalized);
    }
  });
});
...
console.log(`Unique Stories Registered: ${allStories.size}`);

if (duplicates.length > 0) {
  console.log(`\n❌ DUPLICATE STORIES DETECTED (${duplicates.length}):`);
  console.log(duplicates.slice(0, 10).join('\n'));
} else {
  console.log(`\n✅ Zero duplicate stories! Uniqueness verified successfully.`);
}
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Summarize Story curriculum is 100% PERFECT!`);
  process.exit(0);
}
