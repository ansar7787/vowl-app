...
const files = fs.readdirSync(dir).filter(f => f.startsWith('readingSpeedCheck_') && f.endsWith('.json'));

console.log(`Found ${files.length} Reading Speed Check curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'readingSpeedCheck') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'readingSpeedCheck', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "passage", "question", "options", "correctAnswer", "timeLimit", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Reading Speed Check curriculum is 100% PERFECT!`);
  process.exit(0);
}
