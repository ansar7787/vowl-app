...
const files = fs.readdirSync(dir).filter(f => f.startsWith('guessTitle_') && f.endsWith('.json'));

console.log(`Found ${files.length} Guess Title curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'guessTitle') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'guessTitle', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Guess Title curriculum is 100% PERFECT!`);
  process.exit(0);
}
