...
const files = fs.readdirSync(dir).filter(f => f.startsWith('fixTheSentence_') && f.endsWith('.json'));

console.log(`Found ${files.length} Fix the Sentence curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'fixTheSentence') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'fixTheSentence', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "passage", "missingWord", "options", "correctAnswer", "hint", "explanation"];
    ...
    // Verify passage contains missingWord
    if (q.passage && q.missingWord) {
      if (!q.passage.includes(q.missingWord)) {
        invalidOptions.push(`passage "${q.passage.substring(0, 40)}..." does not contain missingWord "${q.missingWord}" in quest ${q.id} in ${file}`);
      }
    }
...
if (invalidOptions.length > 0) {
  console.log(`\n❌ INVALID OPTIONS OR INCORRECT WORDS DETECTED (${invalidOptions.length}):`);
  console.log(invalidOptions.slice(0, 10).join('\n'));
} else {
  console.log(`✅ Zero option or incorrectWord errors! Every correctAnswer is listed in options, and incorrectWord is present in the passage.`);
}
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Fix the Sentence curriculum is 100% PERFECT!`);
  process.exit(0);
}
