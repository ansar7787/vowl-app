...
const files = fs.readdirSync(dir).filter(f => f.startsWith('articleInsertion_') && f.endsWith('.json'));

console.log(`Found ${files.length} Article Insertion curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'articleInsertion') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'articleInsertion', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Article Insertion curriculum is 100% PERFECT!`);
  process.exit(0);
}
