...
const files = fs.readdirSync(dir).filter(f => f.startsWith('shadowingChallenge_') && f.endsWith('.json'));

console.log(`Found ${files.length} Shadowing Challenge curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'shadowingChallenge') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'shadowingChallenge', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "word", "textToSpeak", "options", "correctAnswerIndex", "ipa", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Shadowing Challenge curriculum is 100% PERFECT!`);
  process.exit(0);
}
