$files = Get-ChildItem -Path lib\features\speaking -Recurse -Include *_screen.dart | Select-Object -ExpandProperty FullName

foreach ($file in $files) {
    if (Test-Path $file) {
        $content = Get-Content -Raw $file
        
        $content = $content -replace "(?m)^import 'package:vowl/features/speaking/.*?_explanation_card\.dart';\r?\n?", ""
