...
const files = fs.readdirSync(dir).filter(f => f.startsWith('modifierPlacement_') && f.endsWith('.json'));

console.log(`Found ${files.length} Modifier Placement curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'modifierPlacement') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'modifierPlacement', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "sentence", "correctAnswer", "shuffledWords", "hint", "explanation"];
    ...
    if (q.correctAnswer) {
      ...
      if (allSentences.has(normalized)) {
        duplicates.push(`Duplicate sentence: "${q.correctAnswer}" in ${file} (Quest ID: ${q.id})`);
      }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Modifier Placement curriculum is 100% PERFECT!`);
  process.exit(0);
}
