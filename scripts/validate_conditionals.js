...
const files = fs.readdirSync(dir).filter(f => f.startsWith('conditionals_') && f.endsWith('.json'));

console.log(`Found ${files.length} Conditionals curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'conditionals') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'conditionals', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Conditionals curriculum is 100% PERFECT!`);
  process.exit(0);
}
