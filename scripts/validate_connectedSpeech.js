...
const files = fs.readdirSync(dir).filter(f => f.startsWith('connectedSpeech_') && f.endsWith('.json'));

console.log(`Found ${files.length} Connected Speech curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'connectedSpeech') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'connectedSpeech', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Connected Speech curriculum is 100% PERFECT!`);
  process.exit(0);
}
