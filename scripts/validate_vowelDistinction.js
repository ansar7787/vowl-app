...
const files = fs.readdirSync(dir).filter(f => f.startsWith('vowelDistinction_') && f.endsWith('.json'));

console.log(`Found ${files.length} Vowel Distinction curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'vowelDistinction') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'vowelDistinction', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "word", "textToSpeak", "options", "correctAnswerIndex", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Vowel Distinction curriculum is 100% PERFECT!`);
  process.exit(0);
}
