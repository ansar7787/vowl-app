...
const files = fs.readdirSync(dir).filter(f => f.startsWith('readingConclusion_') && f.endsWith('.json'));

console.log(`Found ${files.length} Reading Conclusion curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'readingConclusion') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'readingConclusion', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Reading Conclusion curriculum is 100% PERFECT!`);
  process.exit(0);
}
