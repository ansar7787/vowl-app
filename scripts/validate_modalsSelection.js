...
const files = fs.readdirSync(dir).filter(f => f.startsWith('modalsSelection_') && f.endsWith('.json'));

console.log(`Found ${files.length} Modals Selection curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'modalsSelection') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'modalsSelection', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Modals Selection curriculum is 100% PERFECT!`);
  process.exit(0);
}
