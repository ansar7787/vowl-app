...
const files = fs.readdirSync(dir).filter(f => f.startsWith('wordLinking_') && f.endsWith('.json'));

console.log(`Found ${files.length} Word Linking curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'wordLinking') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'wordLinking', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "words", "textToSpeak", "correctAnswer", "hint", "explanation"];
    ...
    // Verify words is an array and correct answer pair matches a adjacent combination
    if (q.words && q.correctAnswer) {
      if (!Array.isArray(q.words)) {
        missingFields.push(`words is not an array in quest ${q.id} in ${file}`);
      } else {
        let matchedAdjacent = false;
        for (let i = 0; i < q.words.length - 1; i++) {
          const adj = `${q.words[i]} ${q.words[i+1]}`.toLowerCase().trim();
          if (adj === q.correctAnswer.toLowerCase().trim()) {
            matchedAdjacent = true;
            break;
          }
        }
        if (!matchedAdjacent) {
          invalidOptions.push(`correctAnswer "${q.correctAnswer}" does not match any adjacent combination of words list [${q.words.join(', ')}] in quest ${q.id} in ${file}`);
        }
      }
    }
...
if (invalidOptions.length > 0) {
  console.log(`\n❌ INVALID CORRECT ANSWER OR PAIR DETECTED (${invalidOptions.length}):`);
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Word Linking curriculum is 100% PERFECT!`);
  process.exit(0);
}
