...
const files = fs.readdirSync(dir).filter(f => f.startsWith('paragraphSummary_') && f.endsWith('.json'));

console.log(`Found ${files.length} Paragraph Summary curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'paragraphSummary') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'paragraphSummary', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "passage", "keywords", "options", "correctAnswer", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Paragraph Summary curriculum is 100% PERFECT!`);
  process.exit(0);
}
