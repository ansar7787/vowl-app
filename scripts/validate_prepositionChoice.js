...
const files = fs.readdirSync(dir).filter(f => f.startsWith('prepositionChoice_') && f.endsWith('.json'));

console.log(`Found ${files.length} Preposition Choice curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'prepositionChoice') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'prepositionChoice', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Preposition Choice curriculum is 100% PERFECT!`);
  process.exit(0);
}
