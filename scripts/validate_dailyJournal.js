...
const files = fs.readdirSync(dir).filter(f => f.startsWith('dailyJournal_') && f.endsWith('.json'));

console.log(`Found ${files.length} Daily Journal curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'dailyJournal') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'dailyJournal', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Daily Journal curriculum is 100% PERFECT!`);
  process.exit(0);
}
