...
const files = fs.readdirSync(dir).filter(f => f.startsWith('consonantClarity_') && f.endsWith('.json'));

console.log(`Found ${files.length} Consonant Clarity curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'consonantClarity') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'consonantClarity', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Consonant Clarity curriculum is 100% PERFECT!`);
  process.exit(0);
}
