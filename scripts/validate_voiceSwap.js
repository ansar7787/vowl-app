...
const files = fs.readdirSync(dir).filter(f => f.startsWith('voiceSwap_') && f.endsWith('.json'));

console.log(`Found ${files.length} Voice Swap curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'voiceSwap') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'voiceSwap', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Voice Swap curriculum is 100% PERFECT!`);
  process.exit(0);
}
