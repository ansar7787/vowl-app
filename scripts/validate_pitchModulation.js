...
const files = fs.readdirSync(dir).filter(f => f.startsWith('pitchModulation_') && f.endsWith('.json'));

console.log(`Found ${files.length} Pitch Modulation curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'pitchModulation') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'pitchModulation', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Pitch Modulation curriculum is 100% PERFECT!`);
  process.exit(0);
}
