...
const files = fs.readdirSync(dir).filter(f => f.startsWith('directIndirectSpeech_') && f.endsWith('.json'));

console.log(`Found ${files.length} Direct/Indirect Speech curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'directIndirectSpeech') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'directIndirectSpeech', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "question", "options", "correctAnswerIndex", "correctAnswer", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Direct/Indirect Speech curriculum is 100% PERFECT!`);
  process.exit(0);
}
