...
const files = fs.readdirSync(dir).filter(f => f.startsWith('subjectVerbAgreement_') && f.endsWith('.json'));

console.log(`Found ${files.length} Subject-Verb Agreement curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'subjectVerbAgreement') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'subjectVerbAgreement', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Subject-Verb Agreement curriculum is 100% PERFECT!`);
  process.exit(0);
}
