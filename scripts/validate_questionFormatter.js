...
const files = fs.readdirSync(dir).filter(f => f.startsWith('questionFormatter_') && f.endsWith('.json'));

console.log(`Found ${files.length} Question Formatter curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'questionFormatter') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'questionFormatter', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Question Formatter curriculum is 100% PERFECT!`);
  process.exit(0);
}
