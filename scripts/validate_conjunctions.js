...
const files = fs.readdirSync(dir).filter(f => f.startsWith('conjunctions_') && f.endsWith('.json'));

console.log(`Found ${files.length} Conjunctions curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'conjunctions') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'conjunctions', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Conjunctions curriculum is 100% PERFECT!`);
  process.exit(0);
}
