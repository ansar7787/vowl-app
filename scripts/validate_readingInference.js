...
const files = fs.readdirSync(dir).filter(f => f.startsWith('readingInference_') && f.endsWith('.json'));

console.log(`Found ${files.length} Reading Inference curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'readingInference') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'readingInference', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "passage", "question", "options", "correctAnswer", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Reading Inference curriculum is 100% PERFECT!`);
  process.exit(0);
}
