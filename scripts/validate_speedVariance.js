...
const files = fs.readdirSync(dir).filter(f => f.startsWith('speedVariance_') && f.endsWith('.json'));

console.log(`Found ${files.length} Speed Variance curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'speedVariance') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'speedVariance', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "word", "textToSpeak", "options", "correctAnswerIndex", "targetSpeed", "hint", "explanation"];
...
if (duplicates.length > 0 || missingFields.length > 0 || invalidOptions.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Speed Variance curriculum is 100% PERFECT!`);
  process.exit(0);
}
