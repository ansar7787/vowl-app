...
const files = fs.readdirSync(dir).filter(f => f.startsWith('shortAnswerWriting_') && f.endsWith('.json'));

console.log(`Found ${files.length} Short Answer curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'shortAnswerWriting') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'shortAnswerWriting', got '${data.gameType}'`);
  }
  ...
  data.quests.forEach(q => {
    ...
    const required = ["id", "instruction", "difficulty", "subtype", "interactionType", "prompt", "options", "sampleAnswer", "hint", "explanation"];
    ...
    // Verify options is an array of size 3
    if (q.options) {
      if (!Array.isArray(q.options)) {
        missingFields.push(`options is not an array in quest ${q.id} in ${file}`);
      } else if (q.options.length !== 3) {
        missingFields.push(`options array must have exactly 3 elements, got ${q.options.length} in quest ${q.id} in ${file}`);
      }
    }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Short Answer curriculum is 100% PERFECT!`);
  process.exit(0);
}
