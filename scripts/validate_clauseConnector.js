...
const files = fs.readdirSync(dir).filter(f => f.startsWith('clauseConnector_') && f.endsWith('.json'));

console.log(`Found ${files.length} Clause Connector curriculum files. Beginning audits...\n`);
...
files.forEach(file => {
  ...
  if (data.gameType !== 'clauseConnector') {
    missingFields.push(`Incorrect gameType in ${file}: expected 'clauseConnector', got '${data.gameType}'`);
  }
...
if (duplicates.length > 0 || missingFields.length > 0) {
  ...
  console.log(`🏆 CONGRATULATIONS: Clause Connector curriculum is 100% PERFECT!`);
  process.exit(0);
}
