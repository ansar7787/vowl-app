...
for (let file of files) {
    ...
    if (!content.includes('List<String> _spokenCandidates = [];')) {
        content = content.replace(/(String _spokenText = "";|String _spokenPhrase = "";|String _lastWords = "";)/, '\\n  List<String> _spokenCandidates = [];');
    }
    ...
    if (file.includes('accent_shadowing_screen.dart')) {
        ...
        // Generic rule for repeat_sentence, scene_description, situation, speak_missing, speak_opposite, speak_synonym
        content = content.replace(/(?:final\s+)?bool isCorrect = TextSimilarityHelper\.isMatch\(\s*[^,]+,\s*([^,]+),\s*threshold:\s*([^,]+),\s*\);/,
            'bool isCorrect = false;\n' +
            '    for (var candidate in _spokenCandidates.isEmpty ? [_spokenText] : _spokenCandidates) {\n' +
