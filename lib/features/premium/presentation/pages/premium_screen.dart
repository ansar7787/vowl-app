...
class _PremiumScreenState extends State<PremiumScreen> {
  ...
  void _handlePaymentTimeout() {
    ...
    if (mounted) {
      setState(() {
        ...
        _errorMessage = context.tr('premium.error_timeout', fallback: 'Request timed out', fallback: 'Request timed out');
      });
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          _paymentCompleted = true;
          _paymentSuccess = false;
          _errorMessage = context.tr('premium.error_upgrade_failed', fallback: 'Upgrade Failed', fallback: 'Upgrade Failed');
        });
      }
    }
  }
  ...
  void _handlePaymentFailure(PaymentFailureResponse response) {
    ...
    if (mounted) {
      setState(() {
        ...
        _errorMessage = response.message ?? context.tr('premium.error_generic', fallback: 'An error occurred', fallback: 'An error occurred');
      });
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              Semantics(
                liveRegion: true,
                child: Text(
                  context.tr('premium.processing_title', fallback: 'Processing...', fallback: 'Processing...'),
                  style: TextStyle(
                    fontFamily: 'Outfit',
                    color: Colors.white,
                    fontSize: 16.sp,
              ...
              SizedBox(height: 8.h),
              Text(
                context.tr('premium.processing_subtitle', fallback: 'Please wait while we confirm your purchase.', fallback: 'Please wait while we confirm your purchase.'),
  ...
  Widget _buildCTAButton() {
    final ctaLabel = _isProcessing
        ? context.tr('premium.cta_processing', fallback: 'Processing...', fallback: 'Processing...')
        : context.tr('premium.cta_activate', fallback: 'Activate Premium', fallback: 'Activate Premium');
  ...
  Widget _buildSecureTag() {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Text(
      context.tr('premium.secure_transaction_tag', fallback: 'Secure Transaction', fallback: 'Secure Transaction'),
      textAlign: TextAlign.center,
      style: TextStyle(
        fontFamily: 'Outfit',
