...
class VerifyEmailIconHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      image: true,
      label: context.tr('auth.verify_email_icon_label', fallback: 'Verify Email', fallback: 'Verify Email'),
      child: Container(
...
class VerifyEmailStatusText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          context.tr('auth.verify_email_title', fallback: 'Check your inbox', fallback: 'Check your inbox'),
          style: TextStyle(
            fontFamily: 'Outfit',
            fontSize: 28.sp,
        ...
        Text(
          context.tr('auth.verify_email_description', fallback: 'We sent a verification link to your email. Please click the link to verify your account.', fallback: 'We sent a verification link to your email. Please click the link to verify your account.'),
          style: TextStyle(
            fontFamily: 'Outfit',
            fontSize: 15.sp,
...
class ResendEmailButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // fragments around the number (e.g. '${context.tr('auth.resend_in', fallback: 'Resend in', fallback: 'Resend in')}
    // $secondsRemaining ${context.tr('auth.seconds_unit', fallback: 's', fallback: 's')}') is a worse
    ...
    final label = canResendEmail
        ? context.tr('auth.resend_email', fallback: 'Resend Email', fallback: 'Resend Email')
        : 'Resend in $secondsRemaining s';
    return Semantics(
      button: true,
      label: canResendEmail
          ? context.tr('auth.resend_verification_email_semantic', fallback: 'Resend verification email', fallback: 'Resend verification email')
...
class VerifyConfirmationButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      button: true,
      label: context.tr('auth.verified_confirmation_semantic', fallback: 'Email verified, you can now continue', fallback: 'Email verified, you can now continue'),
      child: OutlinedButton(
        ...
        child: Text(
          context.tr('auth.verified_confirmation', fallback: 'Email Verified!', fallback: 'Email Verified!'),
          style: TextStyle(
            fontFamily: 'Outfit',
            fontSize: 16.sp,
            fontWeight: FontWeight.bold,
...
class VerifyLogoutButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      button: true,
      label: context.tr('auth.cancel_and_logout_semantic', fallback: 'Cancel and log out', fallback: 'Cancel and log out'),
      child: TextButton(
        onPressed: onPressed,
        ...
        child: Text(
          context.tr('auth.cancel_and_logout', fallback: 'Cancel', fallback: 'Cancel'),
          style: TextStyle(
            fontFamily: 'Outfit',
            fontSize: 16.sp,
