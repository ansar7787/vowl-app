...
class _ListeningBaseLayoutState extends State<ListeningBaseLayout> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _audioController = AnimationController(vsync: this, duration: 3.seconds);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<ListeningBloc, ListeningState>(
      listenWhen: (previous, current) {
        if (current is ListeningLoaded) {
          if (previous is! ListeningLoaded) return true;
    ...
    return Container(
      ...
      child: Row(
        children: [
          GestureDetector(
            onTap: () {
              ...
              if (widget.audioUrl!.startsWith('http')) {
                _soundService.playUrl(widget.audioUrl!);
              } else {
                _ttsService.speak(widget.audioUrl!);
              }
              _audioController.forward(from: 0);
            },
            child: Container(
              width: 60.r, height: 60.r,
              decoration: BoxDecoration(gradient: LinearGradient(colors: [theme.primaryColor, theme.primaryColor.withValues(alpha: 0.7)]), shape: BoxShape.circle,
                boxShadow: [BoxShadow(color: theme.primaryColor.withValues(alpha: 0.4), blurRadius: 15, offset: const Offset(0, 5))]),
              child: AnimatedBuilder(
                animation: _audioController,
                builder: (context, child) => Stack(
                  alignment: Alignment.center,
                  children: [
                    SizedBox(width: 50.r, height: 50.r, child: CircularProgressIndicator(value: _audioController.value, strokeWidth: 3, color: Colors.white, backgroundColor: Colors.white.withValues(alpha: 0.2))),
          ...
          Icon(Icons.graphic_eq_rounded, color: theme.primaryColor.withValues(alpha: 0.5), size: 30.r),
        ],
      ),
    ).animate().fadeIn().slideY(begin: -0.2, end: 0);
  }

  Widget _buildCustomHeader(BuildContext context, ListeningState state, int level, double progress, int lives, dynamic theme, bool isDark, ListeningQuest? quest) {
          ...
          if (quest != null && !widget.isAnswered) ...[
            ...
            Padding(
              ...
              child: QuestHintButton(
                used: (state is ListeningLoaded) ? state.hintUsed : false,
                primaryColor: theme.primaryColor,
                hintText: quest.hint,
  ...
  Widget _buildModernFeedbackCard(BuildContext context, ListeningState state, dynamic theme, bool isDark) {
    ...
    final showCorrectAnswer = !success && (state as ListeningLoaded).isFinalFailure;
    final buttonText = success 
        ? "CONTINUE" 
        : ((state as ListeningLoaded).isFinalFailure 
  ...
  VowlMascotState _getMascotState(ListeningState state, int lives) {
