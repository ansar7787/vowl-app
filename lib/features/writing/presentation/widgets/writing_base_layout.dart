...
class _WritingBaseLayoutState extends State<WritingBaseLayout> {
          ...
          if (state is WritingError) {
            ...
            resizeToAvoidBottomInset: false,
              ...
              body: GameErrorWidget(
                ...
                onRetry: () => context.read<WritingBloc>().add(FetchWritingQuests(gameType: widget.gameType, level: widget.level)),
          ...
          return PopScope(
            ...
            child: Builder(
              builder: (context) {
                ...
                final currentQuest = (state is WritingLoaded) ? state.currentQuest : null;
            ...
            body: Stack(
              children: [
                ...
                InkStreak(color: theme.primaryColor.withValues(alpha: 0.15)),
                if (state is WritingLoading) GameShimmerLoading(primaryColor: theme.primaryColor)
                else ...[
                  SafeArea(
                    child: Column(
                      children: [
                ...
                if (widget.isAnswered && state is! WritingGameOver && state is! WritingGameComplete)
                  Positioned(
                ...
                if (_showBriefing)
                  Builder(
                    builder: (context) {
                      final briefing = GameInstructionService.getBriefing(widget.gameType, "Writing", level: widget.level);
  ...
  Widget _buildHeader(BuildContext context, WritingState state, int level, double progress, int lives, dynamic theme, bool isDark, dynamic quest) {
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Text(message, style: TextStyle(fontFamily: 'Outfit', fontSize: 11.sp, fontWeight: FontWeight.bold, color: Colors.blueAccent)),
  ...
  Widget _buildModernFeedbackCard(BuildContext context, WritingState state, dynamic theme, bool isDark) {
    ...
    final showCorrectAnswer = !success && (state as WritingLoaded).isFinalFailure;
