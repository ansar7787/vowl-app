...
class PhotoEmptyState extends StatelessWidget {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: Stack(
              ...
              children: [
                ...
                ).animate(onPlay: (controller) => controller.repeat()).scaleXY(begin: 0.5, end: 1.5, duration: 2.5.seconds).fade(begin: 0.8, end: 0.0),
                
                // Expanding Ripple 2
                Container(
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    border: Border.all(color: const Color(0xFF14B8A6).withValues(alpha: 0.5), width: 3),
                  ),
                ).animate(onPlay: (controller) => controller.repeat()).scaleXY(begin: 0.5, end: 1.5, duration: 2.5.seconds, delay: 1.2.seconds).fade(begin: 0.8, end: 0.0),
          ...
          Text(
            context.tr('vocabulary.photo_prompt', fallback: 'Discover words around you!'),
            style: TextStyle(
              fontFamily: 'Outfit', 
              fontSize: 26.sp, 
  ...
  Widget _buildGlassAction(BuildContext context, {required IconData icon, required String label, required VoidCallback onTap, required bool isDark}) {
    return ScaleButton(
      onTap: onTap,
      child: ClipRRect(
        borderRadius: BorderRadius.circular(40.r), // Pill shape for organic feel
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: const Color(0xFF14B8A6).withValues(alpha: 0.4), blurRadius: 15, offset: const Offset(0, 5)),
              ],
            ),
            child: Row( // Row layout for pill shape
              children: [
