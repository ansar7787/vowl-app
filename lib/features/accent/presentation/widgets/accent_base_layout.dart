...
class _AccentBaseLayoutState extends State<AccentBaseLayout> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AccentBloc, AccentState>(
      listenWhen: (previous, current) {
        if (current is AccentLoaded) {
          if (previous is! AccentLoaded) return true;
          ...
          return PopScope(
            ...
            body: Stack(
              children: [
                ...
                if (state is AccentLoading) const GameShimmerLoading()
                else ...[
                  SafeArea(
                    child: Column(
                      children: [
          ...
          if (quest != null && !widget.isAnswered)
            Padding(
              ...
              child: QuestHintButton(
                used: (state is AccentLoaded) ? state.hintUsed : false,
                primaryColor: theme.primaryColor,
                hintText: quest.hint,
  ...
  Widget _buildPeekingMascot(AccentState state, int lives) {
    final mascotState = _getMascotState(state, lives);
    String message = "Listen carefully! 🎙️";
    if (widget.isCorrect == true) {
      message = "Perfect Accent! ✨";
  ...
  Widget _buildFeedbackCard(BuildContext context, AccentState state, dynamic theme, bool isDark) {
    ...
    final showCorrectAnswer = !success && (state as AccentLoaded).isFinalFailure;
    final buttonText = success 
        ? "CONTINUE" 
        : ((state as AccentLoaded).isFinalFailure 
  ...
  VowlMascotState _getMascotState(AccentState state, int lives) {
