...
class TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= width; x += 4.w) {
      ...
      final double y =
          midY + (math.sin(deflection * math.pi / 2) * 12.h * tiltValue.abs());
      path.lineTo(x, y);
    }
    ...
    if (tiltValue != 0) {
      ...
      for (double x = start; x <= end; x += 4.w) {
        final double distanceToSphere = (x - sphereX).abs();
        final double deflection = math.max(
          0.0,
          1.0 - (distanceToSphere / 60.w),
        );
        final double y =
            midY +
            (math.sin(deflection * math.pi / 2) * 12.h * tiltValue.abs());
