...
class _DailyWordsHomeCardState extends State<DailyWordsHomeCard> {
  ...
  Future<void> _navigateAndRefresh() async {
    ...
    // Refresh state when coming back from the route
    if (mounted) {
      setState(() {});
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return SizedBox(height: 120.h); // Placeholder height
    }

    final int day = _service.currentDay;
    final int streak = _service.streak;
    
    // A subtle pulsating border color or background to draw attention
    final Color primaryAccent = const Color(0xFF6366F1);
    final Color secondaryAccent = const Color(0xFF8B5CF6);
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(
                          child: AutoSizeText(
                            context.tr('home.daily_words_title', fallback: 'Daily 10 Words'),
                            style: TextStyle(
                              fontFamily: 'Outfit',
                              fontSize: 18.sp,
