...
class _ReadingBaseLayoutState extends State<ReadingBaseLayout> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<ReadingBloc, ReadingState>(
      listenWhen: (previous, current) {
        if (current is ReadingLoaded) {
          if (previous is! ReadingLoaded) return true;
          ...
          if (state is ReadingError) {
            ...
            resizeToAvoidBottomInset: false,
              ...
              body: GameErrorWidget(
                ...
                onRetry: () => context.read<ReadingBloc>().add(FetchReadingQuests(gameType: widget.gameType, level: widget.level)),
          ...
          if (quest != null && !widget.isAnswered) ...[
            ...
            Padding(
              ...
              child: QuestHintButton(
                used: (state is ReadingLoaded) ? state.hintUsed : false,
                primaryColor: theme.primaryColor,
                hintText: quest.hint,
  ...
  Widget _buildPassageArea(String passage, dynamic theme, bool isDark) {
    return Container(
  ...
  Widget _buildPeekingMascot(ReadingState state, int lives) {
    ...
    String message = "Read carefully!";
    if (widget.isCorrect == true) {
      message = "Brilliant insight! ✨";
    } else if (lives < 3 && !widget.isAnswered) {
      message = "Check the hint! 📚💡";
    } else if (widget.isCorrect == false) {
      message = "Look back at text! 🔍";
    } else if (state is ReadingGameComplete) {
      message = "Reader Extraordinaire! 🏆";
    } else {
  ...
  Widget _buildModernFeedbackCard(BuildContext context, ReadingState state, dynamic theme, bool isDark) {
    ...
    final showCorrectAnswer = !success && (state as ReadingLoaded).isFinalFailure;
    final buttonText = success 
        ? "CONTINUE" 
        : ((state as ReadingLoaded).isFinalFailure 
  ...
  VowlMascotState _getMascotState(ReadingState state, int lives) {
    if (state is ReadingGameComplete) return VowlMascotState.happy;
    if (state is ReadingGameOver) return VowlMascotState.worried;
    if (state is ReadingLoaded) {
