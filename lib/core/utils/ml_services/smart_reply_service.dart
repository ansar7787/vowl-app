...
class SmartReplyService {
  ...
  Future<List<String>> getSuggestions() async {
    try {
      final SmartReplySuggestionResult result = await _smartReply
          .suggestReplies();

      if (result.status == SmartReplySuggestionResultStatus.success) {
        return result.suggestions;
      } else {
        di.sl<AppLogger>().debug(
          'SmartReplyService: No suggestions available (status: ${result.status.name})',
        );
        return [];
      }
    } catch (e) {
      di.sl<AppLogger>().error(
        'SmartReplyService: Failed to get suggestions',
        error: e,
      );
      return [];
    }
